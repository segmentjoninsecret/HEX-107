...
class M1Inference:
    def __init__(
        ...
        self.action_norm_stats, state_norm_stats = self.get_action_stats(self.unnorm_key, policy_ckpt_path=policy_ckpt_path)
        self.normalizer_state = Normalizer('min_max', state_norm_stats)
        self.action_chunk_size = self.get_action_chunk_size(policy_ckpt_path=policy_ckpt_path)
        self.model_name = self.get_model_name(policy_ckpt_path=policy_ckpt_path)
    ...
    def step(
        ...
        **kwargs
        ...
        state = self.normalizer_state.forward(torch.from_numpy(state)).unsqueeze(0)
        vla_input = {
            "batch_images": [images],
            "instructions": [self.task_description],
            "state": np.array(state) if self.model_name == 'QwenHLM' else None,
            "tags": tags,
        ...
        if step % action_chunk_size == 0:
            if self.direct:
                response = self.vla.predict_action([images], [self.task_description], state, tags)
                normalized_actions = response["normalized_actions"]
            else:
                response = self.client.infer(vla_input)
                normalized_actions = response["data"]["normalized_actions"] # B, chunk, D        
            normalized_actions = normalized_actions[0][:, :7]  
    ...
    @staticmethod
    def get_model_name(policy_ckpt_path):
        model_config, _ = read_mode_config(policy_ckpt_path)  # read config and norm_stats
        return model_config['framework']['name']
